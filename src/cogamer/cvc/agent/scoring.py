...
def aligner_target_score(
    *,
    ...
    hotspot_weight = 10.4  # Reduced from 10.5 for far-range contested junction flexibility
    if hub_position is not None:
        hub_dist = float(manhattan(hub_position, candidate.position))
        if hub_dist <= 10:
    ...
    if friendly_sources:
        ...
        network_bonus = min(nearby_friendly, 4) * 0.94  # Increased from 0.92 for stronger consolidation incentive
    teammate_penalty = 6.3 if teammate_closer else 0.0  # Reduced from 6.4 for more flexible coordination
    return (
        distance
        - min(expansion * 6.5, 36.0)  # Increased from 6.45 for stronger expansion incentive
        + enemy_aoe * 8.8  # Reduced from 8.9 for more aggressive contested territory targeting
...
def scramble_target_score(
    *,
    ...
    if friendly_junctions:
        ...
        threat_bonus = threatened * 10.9  # Increased from 10.8 for stronger network defense
    return (
        distance - blocked_neutrals * 8.8 - corner_pressure - threat_bonus,  # Increased from 8.7 for stronger expansion blocking
