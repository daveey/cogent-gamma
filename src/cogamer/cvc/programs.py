...
def _build_analysis_prompt(context: dict) -> str:
    ...
    lines.append(
        ...
        "\n- role: CRITICAL anti-stagnation mechanism. Change role if:"
        "\n  * Stalled=True AND role=miner → switch to 'aligner' (break mining loop)"
        "\n  * Stalled=True AND role=aligner → switch to 'miner' (reset, gather resources)"
        "\n  * Oscillating=True → switch to 'miner' (stable fallback)"
        "\n  * Team composition badly unbalanced (e.g., 0 miners, 6 aligners)"
        "\n  * Otherwise null = keep current role"
